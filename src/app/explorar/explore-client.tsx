...
export default function ExploreClient() {
  ...
  const [tournaments, setTournaments] = useState<ExploreTournament[]>([])
  ...
  useEffect(() => {
    ...
    async function loadTournaments() {
      ...
      if (error) {
        console.error("[/explorar][client] Supabase returned an error:", error)
        setTournaments([])
        setLoadError(
      ...
      <div className="container-custom space-y-8">
        ...
        {loadError && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
            <h2 className="text-lg font-semibold">No se ha podido cargar el listado</h2>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-10 text-center">
            ...
            <p className="mt-3 text-gray-600">
              {loadError
                ? "La vista pública sigue operativa, pero la conexión al backend no está respondiendo correctamente."
                : "Prueba otra búsqueda o amplía la provincia. Aquí solo salen torneos públicos y publicados."}
            </p>
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
